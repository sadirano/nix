...
- [ ] 🧪 Every wrapper exe exists after init: `o e s y p x g f nix`.
...
- [ ] 🧪 Add a rename (e.g. `g = "search"` - the KEY is the builtin slot, the
      value is the new name), `nix --sync`: the new wrapper appears and
      dispatches by argv0. Reversing it (`search = "g"`) names no builtin slot,
      so it is silently ignored and nothing happens - that is the config being
      wrong, not `--sync`.
...
- [ ] 🧪 An invalid shortcut name leaves the builtin working and installs
      nothing.
- [ ] ⚠️ `--sync` while a wrapper is running (a live `x <alias> <long-cmd>`)
